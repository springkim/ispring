	...
	*/
	namespace String{
		...
		*	@details 예시 입력 :  ";*.cpp;*.h;*.c;*.jpg;;;;;" \n
				      예시 출력 :  {"*.cpp","*.h","*.c","*.jpg"}
		*/
		inline std::vector<std::string> Tokenizer(std::string str, std::string token) {
		...
		*/
		inline std::string GetExtOfFile(std::string path) {
		...
		*	@return 파일의 이름.
		*/
		inline std::string GetNameOfFile(std::string path) {
		...
		*/
		inline std::string GetPureNameOfFile(std::string path) {
			std::string::size_type slash = path.find_last_of("/\\");
			if (slash == std::string::npos) {
				slash = -1;
			}
		...
		*/
		inline std::string PadNum(unsigned int num,unsigned int pad) {
