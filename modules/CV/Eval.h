...
namespace ispring {
	...
	*/
	namespace CV{
		...
		*/
		inline float IOU(cv::Rect a, cv::Rect b) {
			int i = (a&b).area();
		...
		*	@return TP,FP,FN
		*/
		/*inline std::tuple<int,int,int> GetRecallPrecisionSingleClass(std::vector<BoxSE> ground_truth,std::vector<BoxSE> predict,float threshold1=0.5F) {
			std::pair<float, float> ret = { 0.0F,0.0F };
			if (predict.size() == 0 && ground_truth.size() == 0) {
			...
			return std::tuple<int, int, int>(TP, FP, FN);
		}*/
		/**
		...
		*/
		//inline std::map<int, std::pair<float, float>> GetRecallPrecisionMultiClass(std::vector<BoxSE> ground_truth, std::vector<BoxSE> predict, float threshold1) {
		...
		*/
		inline void DrawBoxSE(cv::Mat img, BoxSE box,cv::Scalar c=cv::Scalar(-1,-1,-1), std::string additional="") {
		...
		*/
		inline void DrawColorBoxSE(cv::Mat img, BoxSE rect, cv::Scalar c) {
