...
namespace ispring {
	...
	*/
	namespace CV{
		...
		*/
		inline int GetDegree(int x1, int y1, int x2, int y2) {
		...
		*	@warning 결과는 0~179 사이만 출력 합니다.
		*/
		inline int GetDegree(cv::Point from, cv::Point to) {
		...
		*/
		inline int GetDegreeDistance(int d1, int d2) {
			if (d1 < 0 || d1>179) {
		...
		*	@return 검출된 선들의 집합
		*/
		inline std::vector<std::pair<cv::Point, cv::Point>> FindLineByLeastSquareMethod(std::vector<cv::Point> chain, float threshold1 = 1.0F, int threshold2 = 5) {
		...
		*/
		inline bool PtInTriangle(cv::Point pt, cv::Point t1, cv::Point t2, cv::Point t3) {
			bool b1, b2, b3;
			b1 = InOrOut(pt, t1, t2);
			b2 = InOrOut(pt, t2, t3);
			b3 = InOrOut(pt, t3, t1);
		...
		*/
		inline bool PtInRectangle(cv::Point pt, cv::Point t1, cv::Point t2, cv::Point t3, cv::Point t4) {
			bool b1, b2;
