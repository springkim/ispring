...
namespace ispring {
	...
	*/
	namespace CV{
		...
		*/
		inline cv::Mat Thinning(cv::Mat image) {
		...
		*/
		inline cv::Mat PixelDisconnect(cv::Mat image) {
			if (image.channels() != 1 || image.depth() != 0) {
				ISPRING_VERIFY("This is not binary image");
			}
			if (image.rows <= 3 || image.cols <= 3) {
		...
		*/
		inline std::vector<std::vector<cv::Point>> PixelChain(cv::Mat image) {
		...
		*/
		inline cv::Scalar GetRGB(size_t index) {
		...
		*/
		inline cv::Mat AutoCanny(cv::Mat image) {
		...
		*/
		inline void DisplayImage(cv::Mat image, bool stop = true, bool full = false) {
#ifdef ISPRING_WINDOWS
			cv::Mat rsz;
			cv::Size sz;
		...
		*/
		inline void DisplayImage2(cv::Mat image, bool async = false) {
#ifdef ISPRING_WINDOWS
		...
		*/
		inline cv::Mat Zoom(cv::Mat image, double ratio) {
			if (ratio == 1.0F) {
				return image.clone();
			} else if (ratio < 1.0F) {
		...
		*/
		inline cv::Mat ImageRotateOuter(const cv::Mat image, double degree, cv::Point* out = nullptr) {
		...
		*/
		inline cv::Mat ImageRotateInner(const cv::Mat image, double degree, cv::Point2d base=cv::Point2d(-1,-1)) {
		...
		*/
		inline cv::Mat FitImage(cv::Mat _img, int w, int h,uchar pad_color=144) {
		...
		*/
		inline cv::Mat GlueImage(std::vector<cv::Mat> imgs,uchar pad_color=144) {
			if (imgs.size() == 0) {
				return cv::Mat();
			}
			if (imgs.size() == 1) {
